...
//    int y = 300 ;
    ...
    for (int i=0; i<_vDesc.count; i++) {
        ...
        CGPoint bPoint = CGPointMake(0, tempY+20);
        CGPoint ePoint = CGPointMake(x-20, tempY);
        
        EColumnChartLabel *label = [[EColumnChartLabel alloc]initWithFrame:CGRectMake(-20, 0, 10, 0)];
        [label setCenter:CGPointMake(bPoint.x-115, bPoint.y-30)];
    ...
    int acco = _array.count;
    if (acco>12) {
        acco=12;
    }
    for (int i=0; i<acco; i++) {
        
        EColumnChartLabel *label = [[EColumnChartLabel alloc]initWithFrame:CGRectMake(i*KlineHeight+0, y - 30, 10, 30)];
        [label setTextAlignment:NSTextAlignmentCenter];
        [label setBackgroundColor:[UIColor clearColor]];
...
//    float tempHeight = y * (80.0 / 90.0);
//    float tempHeight1 = y * (20.0f / 90.0f);
    
    float tempHeight = y * (180.0f / 200.0f);
    float tempHeight1 = y * (20.0f / 90.0f);
    
 //   NSLog(@"%f");
	CGContextMoveToPoint(context, p1.x +10, tempHeight-p1.y*tempWidth/tempHeight1-30);
	for (; i<[_array count]; i++)
	{
		p1 = [[_array objectAtIndex:i] CGPointValue];
        CGPoint goPoint = CGPointMake(p1.x+10 , tempHeight-p1.y*tempWidth/tempHeight1-30);
...
- (NSNumber *)maxDataPoint {
    if (maxDataPoint) {
        ...
        //            return 0;
        //        }
        __block CGFloat max = ((NSNumber *) dataArray[0]).floatValue;
        [dataArray enumerateObjectsUsingBlock:^(NSNumber *n, NSUInteger idx, BOOL *stop) {
            if (n.floatValue > max)
                max = n.floatValue;
        }];
        return @(max);
...
- (NSNumber *)minDataPoint {
    if (minDataPoint) {
        ...
        //            return 0;
        //        }
        __block CGFloat min = ((NSNumber *)dataArray[0]).floatValue;
        [dataArray enumerateObjectsUsingBlock:^(NSNumber *n, NSUInteger idx, BOOL *stop) {
            if (n.floatValue < min)
                min = n.floatValue;
        }];
...
-(int)linesSpace
{
    ...
    NSArray *tempArray = @[@5,@10,@20,@40,@80,@100,@200,@500,@1000,@2000,@5000,@10000];
    int  check = 0;
    int D_Value =(int)(max - min);
    int different = D_Value%6 == 0 ? (D_Value/6 + 1):(D_Value/6);
